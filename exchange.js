function SacarReferencia() {    
    ...
    request.open('GET', 'ReferenceJSON?json&type=Series&category=' + categoriaescogida, true);  
...
function SacarDatos() { 
      ...
      var request = new XMLHttpRequest();
      request.open('GET', 'CategoryReferenceJSON?json&type=Series&reference=' + referenciaescogida, true);
      request.onload = function() {
      if (request.status >= 200 && request.status < 400) {
          var data = JSON.parse(request.responseText);
